...
viewTexts model renderables =
    renderables
        |> textComposable [ font 50, fill colorWhilePlaying, align Left ]
            ( 60, model.window.height - margin )
            (if List.length model.pressedKeys == 0 then
                "press keys"

             else
                String.toLower <| keyboardKeyToString model.pressedKeys
            )
