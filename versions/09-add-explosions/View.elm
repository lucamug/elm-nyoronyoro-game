    ...
    let
        newPosition =
            Shared.fixBoundary data.window <|
                Shared.Position
                    (data.explosion.position.x + (data.deltaLife * data.explosionSpeed * sin angle))
                    (data.explosion.position.y + (data.deltaLife * data.explosionSpeed * cos angle))
    in
    circle (positionToCanvasPoint newPosition) (data.sizeMoltiplicator * data.deltaLife * data.explosionSize)


viewExplosion : Float -> Shared.Window -> Explosion.Explosion -> List Renderable
viewExplosion count window explosion =
    let
        data =
            { deltaLife = deltaLife
            , explosion = explosion
...
viewExplosions : Float -> Shared.Window -> List Explosion.Explosion -> List Renderable -> List Renderable
viewExplosions count window explosions renderables =
    renderables ++ List.concat (List.map (\explosion -> viewExplosion count window explosion) explosions)
...
viewCanvas model =
    if model.isPaused then
        ...
        []
            ...
            |> viewExplosions model.count model.window model.explosions
